...
#def mapSeq(ins, refs, outdir, options):
class MapSeq(Target):
    def __init__(self, ins, refs, sampledir, options):
        Target.__init__(self)
        self.ins = ins
        self.refs = refs
        self.outdir = sampledir
    ...
    def run(self):
        ...
        outfile = os.path.join(localTempDir, id)
        
        queryfile = "%s.fa" %outfile
        qf = open(queryfile, 'w')
        qf.write(">%s\n" %id)
        qf.write( "%s\n" %ins.seq.upper() )
        qf.close()
        ...
        files = []
        for ref in refs:
            currOutfile = "%s-%s" %(outfile, os.path.basename(ref).lstrip('chr').split('.')[0])
            if not os.path.exists(currOutfile):
                system("lastz --identity=%f --format=cigar --output=%s --chain %s[unmask,multiple] %s" %(options.identity, currOutfile, ref, queryfile))
            files.append(currOutfile)
        system("cat %s > %s" %(' '.join(files), outfile))
        system("mv %s %s" %(outfile, outdir))
        
class MapSeqs(Target):
    def __init__(self, insertions, options):
    ...
    def run(self):
        ...
        for sample in insertions:
            ...
            sampleSeq = os.path.join(self.options.seqs, sample)
            if not os.path.exists( sampleSeq ):
                sys.stderr.write("File %s does not exists\n" %sampleSeq)
                sys.exit()

            system("mkdir -p %s" %sampledir)
            system("mkdir -p %s" %selfdir)

            insList = insertions[sample]
            for ins in insList:
                id = "%s-%d-%d-%s-%d-%d-%s" %(ins.name, ins.start, ins.length, ins.strand, ins.refstart, ins.reflength, ins.refstrand)
                #MAP to the reference (hg19)
                mapfile = os.path.join(sampledir, id)
                if not os.path.exists(mapfile):
                    self.addChildTarget( MapSeq(ins, refs, sampledir, options) )
                #Map to itself:
                selfmapfile = os.path.join(selfdir, id)
                if not os.path.exists(selfmapfile):
                    self.addChildTarget( MapSeq(ins, [sampleSeq], selfdir, options) )

        self.setFollowOnTarget( Stats(insertions, options) )
...
class Stats(Target): 
    ...
    def run(self):
        ...
        statsFile = os.path.join(self.options.outdir, "stats%d.txt" %self.options.cutoff)
        for sample in self.insertions:
            sampledir = os.path.join(self.options.outdir, sample)
            selfdir = os.path.join(sampledir, "selfAlign")

            insList = self.insertions[sample]
            for ins in insList:
                total += 1
                totallen += ins.length
                id = "%s-%d-%d-%s-%d-%d-%s" %(ins.name, ins.start, ins.length, ins.strand, ins.refstart, ins.reflength, ins.refstrand)
                ...
                mhccigars = getMhcMap(cigars, self.options.start, self.options.end)
                mhcMapped = False
                ...
                bestCigar = getBestMap(cigars, self.options.start, self.options.end)
                ...
                if not bestCigar or percentMapped < self.options.cutoff:
                    label2id['Unmapped'].append(id)
                else:
                    #multiM += multiMapped
                    if multiMapped*100.0/ins.length >= self.options.cutoff: #More than 50% of the bases multimapped
                        stats['Multimapping bases'][0] += 1
                        stats['Multimapping bases'][1] += ins.length
                    #bestCigar != None and percentMapped >= self.options.cutoff:
                    stats['Mapped'][0] +=1
                    ...
                    if multipleCopies and ( (ins.refstart >= bestCigar.tend and ins.refstart - bestCigar.tend <= self.options.dist) or ( bestCigar.tstart >= ins.refstart and bestCigar.tstart - ins.refstart <= self.options.dist) ):
...
def main():
    ...
    #    ins = insertions[sample]
    #    for insertion in ins:
    #        #print "%s\t%d\t%d\t%d\t%s\t%s" %(sample, insertion.refstart, insertion.length, insertion.start, insertion.strand, insertion.seq)
    #        print ">%s.%d.%d.%d.%s\n%s" %(sample, insertion.refstart, insertion.length, insertion.start, insertion.strand, insertion.seq)
    

if __name__ == "__main__":
    from referenceViz.src.mapLargeIndels import *
    main()
